...
def run(base_path, data, work_directory=None, callbacks=[], logging={}, model={}, training={}, config=None, wandb_offline=False):
    ...
    # init logging
    wandb_logger = WandbLogger(**logging, save_dir=work_directory, offline=wandb_offline)
...
def main():
    """
    Função principal para executar a extrapolação.
    Lê os argumentos da linha de comando, carrega o ficheiro de configuração YAML
    e inicia a execução (run).
    """
    
    # 1. Criar o "leitor" de argumentos
    parser = argparse.ArgumentParser()
    
    # 2. Adicionar os argumentos ESPERADOS
    # Argumento obrigatório para o ficheiro de configuração
    parser.add_argument('--config', type=str, required=True,
                        help='Caminho para o ficheiro de configuração .yaml')
    
    # CORREÇÃO 1: O argumento '--offline' deve ser definido ANTES de ler os argumentos.
    # Argumento opcional para correr em modo offline
    parser.add_argument('--offline', action='store_true',
                        help='Executa o wandb (logging) em modo offline.')
    
    # 3. Ler os argumentos da linha de comando
    # parse_known_args() divide os argumentos conhecidos (em 'args') 
    # dos desconhecidos (em 'overwrite_args')
    args, overwrite_args = parser.parse_known_args()
    
    # 4. Carregar o ficheiro de configuração YAML
    yaml_config_file = args.config
    config = load_yaml_config(yaml_config_file, overwrite_args)
    
    # CORREÇÃO 2: Adicionar a flag 'wandb_offline' ao config DEPOIS de carregado.
    # O objeto 'args.offline' será True se a flag --offline for passada, e False caso contrário.
